...
class HandEvaluator:
    ...
    def __search_straight(self, cards):
        ...
        rank = -1
        def straight_check(acc, i): return acc & (bit_memo >> (r + i) & 1) == 1
        for r in range(2, 15):
    ...
    def __search_flash(self, cards):
        ...
        for group in suit_groups.values():
            if len(group) >= 5:
                flush_cards = sorted(
                    group, key=lambda c: c.rank, reverse=True)[:5]
    ...
    def __search_fullhouse(self, cards):
        def fetch_rank(card): return card.rank
        ...
        def max_(l): return None if len(l) == 0 else max(l)
    ...
    def __search_straightflash(self, cards):
        flash_cards = []
        def fetch_suit(card): return card.suit
